...
`.static-gallery` and `.featured-image` still carry their own three-path treatment — `.static-gallery:focus-within` / `.static-gallery__item:focus` recede and restore the set at `grayscale(1) contrast(0.94)`, 68% opacity, 1.035 scale — but **no route renders them any more.** They are live CSS for a dead component; keep them together or remove them together. Clicking the hero, or the `Images` row beneath it, opens a dark full-screen lightbox whose slides are square (`border-radius: 0`, matching every other frame on the site) and whose only chrome is a 2.75rem square close control drawn as a translucent white hairline box on transparent ground, filling on hover — the site's hairline language carried onto a dark field.
