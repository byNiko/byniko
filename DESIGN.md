...
- **Sunk Paper** (`{colors.paper-sunk}`): The empty state behind a project image inside its frame, so an unloaded or missing screenshot reads as a recess rather than a hole. Also the ground of the case-study stage and every thumbnail in its strip.
...
- **Label** (`.t-label`, 600, `wdth` 82, 0.688rem, 0.15em tracking, uppercase, 1.4): Real section headings that name a region and are marked up as such (the `h2`s "Selected work", "Pages", and the case-study body's own section headings), facts-panel keys, form field labels, the breadcrumb, project counts, footer meta. This is the system's only uppercase. It is a heading and labelling role, never a decorative line stacked above a headline.
...
Within a band, the recurring device is a two-column asymmetric split that collapses to a stack: content plus a companion column, with a 3rem–5rem gap, promoted at the `lg` breakpoint. On the home page that is `minmax(0,1fr)` beside `23rem`.

**Long-form pages — case studies and flat CMS pages alike — share one inner container, `.case-shell` at `72rem`, centred.** It is not an arbitrary width: it is what the parts add up to, a `40rem` reading column plus a `4rem` gutter plus a `28rem` companion. Naming the reading track by its measure rather than `minmax(0, 1fr)` is what keeps the companion adjacent to the copy; flat pages previously ran `minmax(0, 1fr)` with the prose capped inside it, which left a 515px column inside a 928px track and opened **493px of dead paper** between the copy and the facts panel at 1440. The facts panel fills its track rather than holding a fixed `20rem`, so nothing is left over at the right edge.
...
- **Focus containment:** While the sheet is open, every body child outside the header *and* the sheet gets `inert` and `aria-hidden`, so the browser's own tab order is already correct without a hand-rolled focus trap. Both must be spared, because the sheet is a sibling of the header rather than a descendant. Escape closes the sheet and returns focus to the toggle; the attributes are removed on close.
- **The sheet is rendered outside `<header>`, and must stay there.** The header carries `backdrop-blur`, and an ancestor with `backdrop-filter` becomes the containing block for its `position: fixed` descendants. Nested inside it, the sheet's `inset: 0` resolved against the 64px header rather than the viewport: it rendered 390×128, so its paper covered only the top of the page and the links floated over live content. Any full-screen fixed overlay on this site has to clear the header for the same reason.
...
CMS rich text is rendered through Tailwind Typography with a `.prose-niko` theme that rebinds every prose variable to system tokens: ink body and headings, green links and quote borders, hairline bullets and horizontal rules, muted captions. Prose headings pick up `wdth` 104 and -0.02em tracking so they belong to the same width family as the rest of the page, and links carry a green underline color. Prose is capped at `--measure-longform` (`min(50ch, 100%)`) on flat pages — wider than the 40ch `--measure-prose` used for interface copy, because long-form reading tolerates a longer line. **Case studies deliberately do not use that token:** `.case-body` is capped at a flat `36rem`, because case-study prose runs at prose-lg's 18px where the same 50ch renders only 54–62 characters. Measured at `36rem`: 58–67.

**Every one of these numbers is tuned to the rendered line, not the nominal one, and the token is not portable across sizes.** Archivo's average advance sits far below the `0` glyph the `ch` unit measures, so a nominal 68ch once rendered at 89–94 characters — well past the 65–75 range. `ch` has now come up short twice and long once on this site. Never set a `ch` measure from the nominal figure, and never carry one to a surface at a different font size without re-measuring there.

### Case-Study Imagery

**Character:** The same recessed frame as the work grid, at two sizes, opening into a full-bleed lightbox.

Case studies carry the whole image set in one **artifact**: a Swiper stage with a four-up thumbnail strip beneath it, linked by Swiper's own `Thumbs` module, with `.case-facts` under that as the caption — a hairline row per fact (What I did, Live site), each present only when the project holds it, and the block absent rather than empty when it holds neither. Beneath that the column closes on the offer — the question in the Title role and a full-width solid green action, differing from the caption rows above it on three axes at once (2rem of air against their 0.7rem, a heading role against Label, and the page's only solid colour) so it reads as a different kind of thing rather than another row. This is the facts panel's established move on the home page reused, not a new device.

The page itself closes on a **pager**: previous and next along the curated `workIndex` order, each end carrying its own Label above its link. The order wraps, so neither end is a dead stop. A project with a single image renders the frame alone: no strip, no counter, no swipe.

Swiper was already a dependency for the lightbox, so the stage costs three modules (`Thumbs`, `FreeMode`, `Keyboard`) plus `A11y` rather than a library — 4.5 kB on the route. **Use it rather than hand-rolling the strip.** A scroll-snap version of this had the bug that pattern always has: with `scroll-snap-align: start` the strip cannot scroll far enough to bring the last thumbnails to the leading edge, so the final three images were unreachable, and `padding-inline-end` does not extend a grid container's scrollable area. Swiper positions from the active index instead of from scroll position.

Two things this arrangement needs and does not get for free. Grid children must carry `min-width: 0` — Swiper writes pixel widths onto its slides, which becomes a min-content floor and pinned the stacked column at 36rem inside a 350px track, scrolling the page sideways at 390. And the stage's frame must focus itself in its click handler before opening the lightbox: Swiper preventDefaults `mousedown` to drive dragging, which suppresses the focus a click normally moves to a button, so the lightbox captured `body` as its opener and returned focus to nowhere on Escape.

The case-study hero is `.hero-frame`, and it carries the pointer/keyboard/touch trio in the same shape as everything else: the 1.02 scale is gated on `@media (hover: hover) and (pointer: fine)`, `.hero-frame:focus` reaches it from the keyboard and from an iOS tap, and `prefers-reduced-motion: reduce` drops the scale and collapses the transition to 1ms.

The thumbnail strip is **muted, never desaturated**: inactive thumbs sit at 55% opacity with no filter, and the one Swiper marks `swiper-slide-thumb-active` returns to full opacity inside a green outline. Stripping the colour made one frame hard to tell from another, which is the index's only job — and full desaturation belongs to the work grid, where it is the site's single authored moment. State comes from Swiper's own class, so there is one source of truth rather than a second index kept in React.

The strip runs at `slidesPerView: 4.3`. The fraction is load-bearing: the sliver of the next thumbnail is what says the set continues, where a whole number ends flush and reads as the end. It does **not** use `centeredSlides` — `centeredSlidesBounds` does not clamp against a fractional `slidesPerView`, so the strip sat centred on thumbs 1–3 even at rest and clipped the first thumbnail to 62% visible. Following the selection is what centring was reached for, and `autoScrollOffset: 1` on the stage does that directly: measured, the active thumb is fully visible at all eight positions and the strip pages only when it must. Clicking the stage opens a dark full-screen lightbox whose slides are square (`border-radius: 0`, matching every other frame on the site) and whose only chrome is a 2.75rem square close control drawn as a translucent white hairline box on transparent ground, filling on hover — the site's hairline language carried onto a dark field.
...
**The Offer Follows the Proof Rule.** A case study's call to action sits in the artifact column above 72rem and after the story below it — rendered twice, `display: none` on the copy that does not apply, so exactly one is ever in the accessibility tree. It is not a stylistic choice: stacked, the column sits above the body, and an offer placed there asks before the work has finished proving anything. Measured, the single-placement version pushed the story from y=759 to y=902 on an 844px phone, off the first screen entirely. Any element that is persuasive rather than evidential inherits this rule.
...
**The Stylesheet Ownership Rule.** A component's styles live in the stylesheet imported by the component that renders the element — never in a sibling that happens to import them. This has now failed twice on the same element. `.close-modal` sat in `GalleryStatic/style.css` while `ModalContext` renders it; the first time, deleting a stylesheet dropped the lightbox's positioning, and the second time, a route simply ceasing to render `GalleryStatic` stopped that CSS from loading at all and shipped the close control as a 20px unstyled glyph — under the 24x24 target floor, ink-dark on a near-black scrim, pinned to the wrong edge. It now lives in `ModalContext/style.css`, and `GalleryStatic` and `FeaturedImage` have both been deleted rather than left in the tree unrendered — an unrendered component whose stylesheet others depend on is the trap, and keeping one "just in case" is how this happened twice. Nothing about the rendered page looks wrong in the source when it breaks; only loading the page catches it.
