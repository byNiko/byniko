...
Case studies carry a **featured image** in the sticky artifact column, with `.case-facts` beneath it as its caption: a hairline row per fact — What I did, Live site — each present only when the project holds it, and the whole block absent rather than empty when it holds neither. Below the story, the remaining images render as the **static gallery**, an auto-filling grid of `minmax(min(100%, 15rem), 1fr)` tiles that drops to a fixed 2 columns below `30rem`; the 15rem minimum resolved to a single column on a phone and stacked seven tiles into 1,627px of scrolling. The gallery receives the full lightbox set plus a `from` offset rather than a sliced array, so the hero occupying slide 0 does not shift every tile's index by one.

**The count is never substituted for the tiles.** A pass that replaced this grid with a facts row reading "8 images" was reverted: this is an Experience surface, and putting the artifacts behind a modal is the same fault as burying them under a hero — a lightbox cannot be scanned, only decided into.
