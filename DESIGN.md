...
The thumbnail strip carries the same recede-and-restore: every thumb sits at `grayscale(1) contrast(0.94)` and 55% opacity, and the one Swiper marks `swiper-slide-thumb-active` returns to full colour inside a green outline. State comes from Swiper's own class, so there is one source of truth rather than a second index kept in React. Clicking the stage opens a dark full-screen lightbox whose slides are square (`border-radius: 0`, matching every other frame on the site) and whose only chrome is a 2.75rem square close control drawn as a translucent white hairline box on transparent ground, filling on hover — the site's hairline language carried onto a dark field.
